...
|------|------|-------|----------|
| ~~2026-02-25~~ | ~~**Migrate PB from Onboarding to Competitions Hub** — удалить ввод PB из OnboardingWizard (уже реализовано в 4.262)~~ | ~~Track 4.262~~ | ~~Done~~ |
...
| 2026-02-20 | **PB Admin: Cascade behaviour для athlete_id relations** — сменить `restrict` → `cascade` на коллекциях: training_logs, test_results, daily_checkins, seasons, achievements. Нужна ручная настройка в PB Admin | **Track 4.268** | High |
| ~~2026-02-20~~ | ~~**`user_id` field на athletes** — прямое связывание user ↔ athlete record (сейчас lookup через coach_id + getSelfAthleteId workaround)~~ | ~~**Track 4.268**~~ | ~~Done~~ |
...
| 2026-02-22 | **Project-wide `pb.filter()` migration** — 40+ мест в 15 сервисах используют string interpolation вместо `pb.filter()`. SQL injection risk. Skills: `api-security-best-practices`, `lint-and-validate` | **Track 4.268** | 🔴 High |
...
| 2026-02-23 | **Push delivery via CF Worker** — PB hook при создании notification → вызов CF Worker push-batch. Сейчас Worker задеплоен но не вызывается | **Track 4.268** | 🔴 High |
...
| 2026-02-24 | **CoachDashboard refactor** — 390 LOC inline в `dashboard/page.tsx`, вынести в отдельный `CoachDashboard.tsx` для maintainability | **Track 4.268** | Low |
| ~~2026-02-24~~ | ~~**WeekConstructor hooks extraction** — Успешно выполнено в рамках Track 4.266~~ | ~~Track 4.266~~ | ~~Done~~ |
...
| ~~2026-02-24~~ | ~~**Автоматизация QA Smoke портов** — `pnpm preview` может использовать другой порт (например, 50782), если 3000 занят. Прописать жесткий порт или автоматизировать остановку сервера~~ | ~~**Track 4.268**~~ | ~~Done~~ |
| ~~2026-02-26~~ | ~~**[timezone] planResolution timezone** — передавать IANA timezone из user-профиля в `planResolution.ts → todayForUser(tz)`. Сейчас UTC-only, атлеты в CN/EU могут получить ±1 день сдвиг для standalone планов~~ | ~~**Track 4.268**~~ | ~~Done~~ |
...
| 2026-02-27 | **[log_mode] Передавать log_mode при старте тренировки** — API уже готов (`getOrCreateLog` / `createTrainingLog` принимают `LogMode`), но вызывающий код в `AthleteTrainingView.tsx` не передаёт: `handleStartFocus` → `'live'`, пост-фактум Express → `'post_express'`. Реализовать в UX-редизайн треке | UX-Redesign трек | Medium |
| 2026-02-27 | **[typescript] Строгие типы для PlanExercisesRecord** — `PlanExerciseWithExpand extends RecordModel` наследует `[key: string]: any`, из-за чего TypeScript не поймал `pe.is_warmup` (несуществующее поле). Заменить на точный интерфейс с известными полями, убрать `RecordModel` из базы | **Track 4.268** | Low |
